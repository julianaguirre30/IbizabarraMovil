...
(function () {
	...
	$(function () {
		...
		document.querySelector("#submit").addEventListener("click", e => {
			...
			let cantidad = document.querySelector("#numero").value;
			...
			let url = `https://api.whatsapp.com/send?phone=${telefono}&text=
				Hola Ibiza Barra Movil soy%0A
				  *${cliente}*%0A%0A
				Fecha a consultar%0A
				  *${fecha}*%0A%0A

				Cantidad de invitados%0A
				  *${cantidad}*%0A%0A
			
				 Evento a consultar%0A
				  *${servicio}*`;
		  
			if (cliente === "" || fecha === "" || servicio=== "" || cantidad=== "") {
			  resp.classList.add("fail");
			  resp.innerHTML = `Faltan ingresar datos`;
			  return false;
			}
			resp.classList.remove("fail");
			resp.classList.add("send");
			resp.innerHTML = `Su consulta fue enviada`;
